...
- **`wall_clock_seconds` is now re-measured after the Aggregator returns**, before the markdown report is rendered. The previous design captured wall-clock at Aggregator-dispatch time, so the metric undercounted by the Aggregator's own thinking and writing time (typically 5–9 minutes). On the pytorch-trainer re-run, the dispatch-time value was 680s while the true end-to-end wall was 1205s (44% undercount). Setup step 3 now captures `started_at_unix_seconds` for arithmetic, and a new "Re-measure end-to-end wall_clock_seconds" step between the Aggregator return and the markdown render computes `now - started_at_unix_seconds` and updates `metadata.wall_clock_seconds` in both `final-report.json` and the rendered markdown.
