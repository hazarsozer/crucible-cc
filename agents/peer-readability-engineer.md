...
```json
{
  ...
  "evidence": { "path": "tests/fixtures/nextjs-auth/app/auth/session.ts", "line_start": 22 },
...
Notice: every required field present, `persona`/`stage`/`model_used` match the frontmatter, `score` agrees with the verdict (8/10 with one medium finding is `concerns`, not `block`), `summary_quote` is under 500 chars, `findings` has exactly the issues that belong to this lens, and `stage_handoff_notes` explicitly defers the out-of-scope concerns (localStorage, input validation) to the right downstream personas. Begin your response with `{`, end with `}`, and emit nothing else.
