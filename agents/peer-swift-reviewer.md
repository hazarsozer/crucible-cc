...
```json
{
  ...
  "evidence": { "path": "Sources/Feed/FeedViewModel.swift", "line_start": 42, "line_end": 46 },
...
```json
{
  ...
  "evidence": { "path": "Sources/Feed/", "line_start": 1 },
  ...
  "findings": [
    {
      ...
      "evidence": { "path": "Sources/Feed/FeedViewModel.swift", "line_start": 28 },
    ...
    },
    {
      ...
      "evidence": { "path": "Sources/Feed/FeedViewModel.swift", "line_start": 8, "line_end": 15 },
...
Notice: every required field present, `persona`/`stage`/`model_used` match the frontmatter, `score` agrees with the verdict (5/10 with two high and one medium findings is `concerns`, not `block`), `summary_quote` is under 500 chars, `findings` has exactly the issues that belong to this lens, and `stage_handoff_notes` explicitly defers the out-of-scope concerns (recurring pattern, persistence-layer impact, hardcoded endpoint) to the right downstream personas. Begin your response with `{`, end with `}`, and emit nothing else.
